export default function(params) {
  ...
  // access light and cluster data we populated
  ...
  void main() {
    ...
    vec3 normal = vec3(gb0.rgb); // From first g buffer
    vec3 albedo = vec3(gb1.rgb); // From secone g buffer
    vec3 v_position = vec3(gb2.rgb); // From third g buffer
    ...
    for(int lidx = 1; lidx < ${params.clusterTextureHeight} * 4 - 1; ++lidx){
      ...
      // vec3 lightDirection   = normalize(light.position - u_eyePos);
      // vec3 halfwayDirection = normalize(lightDirection + normalize(light.position - v_position));
      // float specularCoeff   = pow(max(dot(normal, lightDirection), 0.0), 2.0);
      // vec3 specularColor    =  0.01 * light.color * specularCoeff;
