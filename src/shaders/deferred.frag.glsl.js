export default function(params) {
  ...
  void main() {
    ...
    vec3 norm = vec3(position[3], color[3], 0);
    color[3] = 1.0;
    position[3] = 1.0;
    float normZ = sqrt(-(norm[0] * norm[0] + norm[1] * norm[1] - 1.0));
    norm.z = normZ;
    norm = normalize(norm);
    vec4 normal = vec4(norm, 0);*/
    vec4 position = texture2D(u_gbuffers[0], v_uv);
    vec4 color = texture2D(u_gbuffers[1], v_uv);
    vec4 normal = texture2D(u_gbuffers[2], v_uv);

    vec3 fragColor = vec3(0.0);

    vec4 screenSpace = u_viewProjectionMatrix * position;
    ...
    for (int i = 0; i < ${params.maxNumLights}; ++i) {
      ...
      fragColor += color.xyz * (lambertTerm + specularTerm) * light.color * vec3(lightIntensity);
    }

    const vec3 ambientLight = vec3(0.025);
    fragColor += color.xyz * ambientLight;
