export default function(params) {
  ...
  void main() {
    ...
    // Same lighting as before! This just changes where we read the initial data from.
    // Only change, changed 'v_position' references to 'position'

    vec3 fragColor = vec3(0.0);

    // Get cluster info for debug. This was super helpful.
    vec3 debug_clusterColor = PositionToClusterVec3(position);
    debug_clusterColor.x /= u_x_slices;
    debug_clusterColor.y /= u_y_slices;
    debug_clusterColor.z /= u_z_slices;

    int clusterIdx = PositionToCluster(position);
